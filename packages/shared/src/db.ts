...
export class db {
  ...
  public addUser(
    ...
    return new Promise((resolve, reject) => {
      this.connection.run(
        `INSERT INTO users (userID, username, discriminator, avatarURL, messages, xp) VALUES (?, ?, ?, ?, 0, 0)`,
  ...
  public updateUser(
    ...
    return new Promise((resolve, reject) => {
      this.connection.run(
        `UPDATE users SET username = ?, discriminator = ?, avatarURL = ?, messages = ?, xp = ? WHERE userID = ?`,
        [username, discriminator, avatarURL, messages, xp, userID],
        (err) => {
          if (err) {
            reject(err);
          }
